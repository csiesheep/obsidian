...
- 包廂頁（2026-09-14，方案 C）：車次壓在 ComfyUI 畫的車票上（`public/art/ticket.jpg`），狀態是紅色戳章；乘客名單改成編號表格配頭像與狀態籤；列車長設定收成一張卡。首頁桌機版不再貼底，一屏放完。設定頁拿掉三行提示。頭像重畫候選在畫布第六頁等車主逐一確認（16 張過檢，小四／瑪戈／辛格／阿彪未過）。
- 乘客定案十五位（2026-09-14）：周太太、林小姐、陳老闆、娜塔莎用重畫版；王秘書、小四、白老師、伊凡、史密斯、辛格、阿米爾、卡洛斯、巴克、花子、羅莎維持原版；金老先生、瑪戈、梅姨、傅先生、阿彪退場。
- 車廂頁（2026-09-14，方案 C＋徽章）：一排座位、排高固定；輪到誰或攻守兩人放大到 72px，其他 46px；立場靠圈色與右上角徽章（劍／盾／一橫），第二行寫行李或攻守計數；衝突卡改成左右頭像、中間「劍 N vs N 盾」大數字，不再列每個人的選擇；行李下方是「車廂閒話」：氣泡對話、置中的系統事件、包廂模式有快捷句與輸入列。揭示頁縮到一屏並保留頁首列。設計稿在畫布第八、九頁。
- 座位改圓桌（2026-09-14，畫布第十頁方案 A 修三）：你在正下方、其他人順時針；輪到誰金圈放大、背後桌面淡金光；衝突時攻方背後紅光、守方藍光，幫攻紅圈、幫守藍圈、旁觀變暗，沒有箭與徽章；圓心寫站次與回合，衝突時寫「誰對誰」與「劍 N vs N 盾」。亮出的物品／行當貼在人旁（紅框加攻、藍框加守、灰框其他），點開說明。點任何人物彈出「你對他所知」：幫會、行當、行李（看過的顯示圖，沒看過的用問號補齊件數），由公開紀錄與私下得知的事實依時間重播，凡是看不見的移動就放棄那個人的已知清單。
- 車廂補三項（2026-09-14）：衝突算完後原本的衝突卡留在畫面，寫主動方贏了／輸了（或平手、被攔下、取消）、後續（看了幫會與行當／拿走一件／抽一張，與你有關的補一句），按「知道了」才消失；車廂閒話可收合，收起只剩最後一句預覽，包廂預設開、單人預設收；最下方新增「行車紀錄」，收起一行預覽，展開依站分組，行為人小頭像、涉及物品小圖（紅藍框）。設計在畫布第十一頁。
- 查錯一輪（2026-09-14）：新增 `tests/coverage.mjs`（每個人數跑機器人對局，每步檢查物件守恆、上限、行當使用、視圖不洩漏，統計每種物品文字與行當能力是否觸發）；96 局 0 引擎錯誤、0 不變量違反，所有物品與行當都觸發過。修了三個客戶端問題：自動對局被結果卡卡住；包廂模式伺服器代答後客戶端殘留選取造成引擎拒絕；包廂模式被拒絕的動作訊息寫到看不見的頁面。結算頁加「你贏了／你輸了」。規則頁兩處英文所有格修正。本機 Node 24.19 跑重負載會 V8 崩潰（feedback sentinel／segfault），與遊戲無關；跑測試前先清掉殘留的 wrangler／workerd。

## DLC 構想（2026-09-14，腦力激盪，未決定、未實作）

依「規則擴充 → 模式 → 內容與社群」排列。物品與行當在引擎裡是資料表，前四項成本最低。

1. **第二節車廂（新行李包）**：加 6 件新物品，例如「車票剪」（換出去時看對方一件並可換位置）、「假鬍子」（衝突中你的行當視為未亮出）、「電報」（公開宣告一件物品在誰手上，對錯都要付代價）。難度低，只加資料表與文案。
2. **新行當 · 車上人員**：列車長（一局一次：指定下一站誰先走）、餐車侍者（每次交換後可偷看牌堆頂）、扒手（幫攻贏時多拿一件）、記者（攤牌錯誤時你不算輸）。難度低到中，扒手與記者要碰結算。
3. **第三方 · 便衣**：單獨一人的第三陣營，目標是在任一幫會攤牌前收齊三件不同的武器類物品。打破雙方對稱，多人局更緊張。難度中，要改分牌、攤牌結算與機器人信念。
4. **站站有事（事件卡）**：每過一站抽一張：查票（全員亮出一件）、停電（這一站不能動手）、臥鋪對調（左右鄰座各傳一件）、餐車（牌堆頂兩張翻開）。難度低，事件是回合開頭的一個強制動作。
5. **黑夜列車 · 高難模式**：走私預設開、不公開衝突亮出的物品、行車紀錄只留最近一站、AI 在「難」之上加一檔會記仇的「狠」。純設定與機器人參數，難度低。
6. **兩人局 · 對坐**：一對一加兩個公開的假人座位（行李面朝上、不行動、可被攻擊），門檻從三人降到兩人。難度中，主要是規則調整與平衡模擬。
7. **戰役 · 十二站到終點**：十二個劇本，各有預設角色、起始行李和特殊勝利條件（護送某件物品到終點、第 N 站前找出臥底），單人打通解鎖新頭像。難度中，需要劇本格式與存檔。
8. **乘客名冊擴充包**：每包十位新乘客（其他時代裝扮、動物擬人、經典偵探致敬），附各自的閒話語氣。用現有 ComfyUI 流程生成，難度低；純外觀。
9. **私人包廂 · 社群功能**：好友清單、常駐房間代號、賽季勝率與「最會騙人的乘客」榜、把一局的行車紀錄分享成一張圖。難度中到高，要帳號或穩定的匿名 ID 與 KV 儲存。
10. **回放與旁白**：一局結束後「重看這班車」：圓桌逐步重演、全程顯示每個人的真實幫會，AI 乘客的閒話當旁白；兼作複盤與教學。難度中，行車紀錄已有完整事件流，主要是播放器 UI。

建議先做 4（站站有事）與 8（乘客名冊擴充包）：一個改變每局節奏，一個把跑順的美術流程變成可重複產出的內容，都不動核心規則。車主選定後先出設計再實作。

## 擴充設計稿與車主選定（2026-09-14 設計，2026-09-15 選定，未實作）

設計畫布第十二頁「擴充包 2 · 十個新行當」、第十三頁「擴充包 · 十件新行李」。圖由本機 ComfyUI 以原版物件提示詞畫成（scratchpad `art10/`、`art11/`）。

### 選定

- **物品：金條**（Gold Bar）：你在衝突中輸了時，可以把金條交給贏家代替；他拿走金條，不能再看、也不能拿別的。引擎：衝突結算、贏家選擇前加一個新選擇。
- **行當：雙面人**（Double Agent，一局一次）：有人要看你的幫會牌時（單片鏡、衝突勝方看牌等），你可以讓他看到另一邊。整套唯一的假情報；會碰到已知清單與點人物彈出的資訊。
- **行當：搬運工**（Porter，隨時有效）：你的行李上限 **+2**（設計稿原為 +1，車主改）。
- **行當：賭徒**（Gambler，隨時有效）：身為攻方或守方，大家表態之後**擲骰子，隨機 +0／+1／+2**（設計稿原為翻牌堆頂，車主改）。需要種子亂數，行車紀錄與衝突結果要顯示擲出的點數。

### 未選（留作參考）

- 行當：列車長（查票，全員亮一件）、記者（跟著看勝方看到的幫會與行當）、偵探（猜幫會，對看行李、錯被看）、電報員（互看幫會）、扒手（幫攻贏時摸一件）、魔術師（盲換各抽一件）、餐車侍者（先看遞來的東西再決定收不收）。
- 物品：胡椒罐（守方讓攻方一件物品不算）、保險單（被拿走時從牌堆補一件）、望遠鏡（換出去時看行當）、報紙（換出去時對方亮一件）、安眠藥（收下的人下一回合只能過）、車門鑰匙（抽對方一件放牌堆底）、手銬（收下的人下一回合前不能動手或幫手）、贗品（被看到時由持有者說是懷錶或玉印）、可疑包裹（不能拒收，持有者當守方時守 −1）。

### 待辦

- [ ] 平衡：先用機器人模擬（搬運工 +2 在三、四人局上限小時影響大；賭徒平均 +1，與打手／武師相當但波動大；金條與雙面人對勝率的影響）。
- [ ] 決定擴充怎麼開：新行當與原本十個一起洗、或包廂設定裡可勾選；金條直接加入牌堆（22 件）。
- [ ] 實作：引擎資料表與步驟（金條選擇、雙面人假情報、賭徒擲骰）、機器人決策、i18n、規則頁、`tests/coverage.mjs` 期望清單、卡圖裁切進 `public/art/`。

### 平衡模擬（2026-09-15）

- 規則定案：**雙面人**說謊時，對方看到另一個幫會，行當顯示成盒子裡隨機一張沒發出去的行當；能力算用掉，不公開。
- 做法：分支 `dlc-balance`（未提交、未部署）在引擎加預設關閉的擴充開關 `options.dlc = { gold, double, porter, gambler }`，機器人會用這四張牌；`tests/dlc_balance.mjs` 跑普通難度機器人，六組（原版、四張各自單開、全開）× 3–10 人 × 每格 600 局。有 4 格（原版、金條、搬運工、全開的 10 人局）因本機 Node 當掉沒跑完。全開覆蓋測試 3–10 人各 15 局：0 引擎錯誤、0 不變量違反；原有 33 個單元測試全過。
- 指標：拿到某行當的座位最後在贏的一邊的比率，減全桌座位平均（約 44%）；誤差約 ±1 個百分點。原版對照：算命師 +7.2、打手 +5.5、武師 +0.9……外交官 −5.4。
- **賭徒：偏強。** 單開 +8.8（全行當最高），全開 +6.4；自己參與的衝突贏 60%；每局衝突 31.0 → 33.8。攻守都能擲、平均 +1，等於打手加武師合一。
- **雙面人：剛好。** 單開 +0.3、全開 +0.2，每局說謊約 0.5 次。
- **搬運工：略弱、存在感低。** 單開 −1.2、全開 −1.9，在原版範圍內（外交官 −5.4）；只有 15% 的局真的超過一般上限。
- **金條：不強，但讓局變長。** 開局持有者勝率 43.1%（全桌 45.0%，差異不顯著）；平均每局付出 1.4 次、在車上流轉；局長 89.8 → 95.4 回合，7–9 人局獨自下車 16% → 12%。
- **四張全開：** 兩幫勝率不變（39/40），攤牌說錯 22% → 25%，局長 +8%（96.8 回合），衝突 +11%。
- 待決定：賭徒要不要削弱（候選：只在攻方擲，平均 +1、約等於打手；或攻守都擲但改成 0/+1）；搬運工要不要加強或維持；金條拉長局數是否可接受。改完再跑同一套模擬。
- 車主決定（2026-09-15）：**搬運工 A**：上限 +2，另外衝突輸了被拿行李時由搬運工挑給哪一件（用時翻開行當）。**雙面人只保留被動說謊**：主動亮牌在只有雙面人能亮牌的情況下會自曝，放棄。**賭徒改 +0／+2**（平均仍 +1，模擬中）。
- 第二輪模擬（2026-09-15，同一套、每格 600 局；賭徒缺 9 人局一格）：**賭徒 +0／+2：+6.7**（原 +8.8），仍是全行當最高，自己參與的衝突贏 59%；平均 +1、攻守都能用的本質沒變。**搬運工 A：−0.8**（原 −1.2），強度幾乎不變，但存在感大增：拿到它的局平均用 2.1 次挑行李。對照組各輪之間浮動約 ±2（打手 +1.9～+5.5、算命師 +2.4～+7.7），所以賭徒連三輪最高才算確定偏強。下一步候選：賭徒只在攻方擲 +0／+2。
- **定案（2026-09-15）**：金條、雙面人（只被動說謊，說謊時行當顯示成盒中另一張）、搬運工 A（上限 +2＋被拿行李時自己挑）、賭徒（攻守皆可，+0／+2）。賭徒偏強（約 +6.7）車主接受，不再削。規則已在分支 `dlc-balance` 的引擎與機器人裡（未提交、未部署）；尚缺：介面（新步驟：付金條、說謊、挑行李、擲硬幣顯示）、i18n、規則頁、包廂設定開關、卡圖裁切進 `public/art/`。
- **擴充上線（2026-09-15）**：「擴充：車上人員」可玩。單人設定頁與包廂列車長設定各一個開關（開了下面列出四張小卡）；揭示頁、說明卡用新圖（`public/art/trade_double|porter|gambler.jpg`、`item_gold_bar.jpg`，賭徒改畫硬幣）。新畫面：衝突輸家手上有金條時問「交出金條／不交」；雙面人被看幫會牌（單片鏡或衝突勝方看牌）時選「給他看真的／讓他看到另一邊」；搬運工被拿行李時自己挑一件；賭徒的硬幣點數顯示在圓桌小圖（+0／+2）、結果卡與行車紀錄。沒有那張牌的人由畫面自動回答，不停頓、不洩漏。規則頁中英各加「擴充」一段。設計在畫布第十五頁。驗證：39 個單元測試、全開覆蓋測試、瀏覽器五個場景截圖、自動對局跑完一局、本機包廂發車確認選項傳入；部署後逐檔比對一致。提交 3218d00、12b93d1（main）。
- **站站有事 · 事件卡（2026-09-15，設計稿，未實作）**：車主選定每站抽一張，六種事件加無事各佔一半（牌疊十二張：餐車、海關驗關、鍋爐加壓、暗號、車廂廣播、熄燈，加六張無事，抽完重洗），第二站開始，獨立開關。設計在畫布第十六頁（開關與事件疊、站首事件卡、無事的一行、海關、暗號、鍋爐與熄燈在衝突裡、行車紀錄）；事件圖七張在 scratchpad `art15/`（熄燈重畫過一次）。待確認：車廂廣播要不要一併報皮箱數。確認後先跑模擬看局長與攤牌正確率再實作。
- **站站有事上線（2026-09-15）**：獨立開關「站站有事」（單人設定頁與包廂列車長設定各一個）。第二站起每站抽一張：牌疊十二張（餐車、海關驗關、鍋爐加壓、暗號、車廂廣播、熄燈，加六張無事），抽完重洗。事件卡在站首跳出、按「知道了」才繼續；無事只記一行。海關與暗號是同時動作的步驟（暗號直接點圓桌上的人指認）；鍋爐加壓與熄燈整站有效，車次列一直掛著標籤。車廂廣播會一併報皮箱（沒有皮箱時不提）。行車紀錄：海關與暗號的「開始」與「結果」各記一筆（結果另記一筆，不回頭改舊紀錄，否則已經讀過的那行不會更新）。圖七張在 `public/art/event_*.jpg`。驗證：39 個單元測試、開著事件的覆蓋測試（0 引擎錯誤，六種事件與無事都觸發、暗號亮牌 21 次／平手 11 次）、瀏覽器四個畫面截圖（含修掉「亮牌卡片印出 null」的 bug）、自動對局跑到第 20 站無錯誤；平衡模擬每格 400 局：局長 89.3 → 87.9 回合、攤牌說錯 22% 不變、兩幫勝率不變，確認事件不影響平衡。部署版本 cea73b4d，逐檔比對一致。提交 07fadd9（main）。

## 新手教學與開場／結局動畫（2026-09-16 上線）

- **教學**（設計在畫布第十七頁，結構參考自家的天地會）：首頁「單人上車」下方一行「第一次玩？三分鐘教學」（`?tutorial`，跑過一次後縮成小字）。教學頁五步配現成圖，按鈕進導覽對局（`?play=tutorial`）。導覽對局固定四人：你是林小姐（鐘樓會，行當保鏢——刻意選不會跳出未教過提示的行當），同會花子，對面伊凡與陳老闆；發牌固定（你：懷錶＋單片鏡；花子：懷錶；陳老闆：懷錶＋匕首；伊凡：搜查令）。機器人照劇本走，偏離劇本就切回正常 AI。嚮導六張：揭示頁、用單片鏡探人、看到情報、被動手、對陳老闆動手拿第三只懷錶、攤牌點名花子。車主定案：**固定讓玩家贏**、**嚮導可隨時關掉**（關掉後就是普通單人局）。結語頁：你學會了、這一局發生過什麼三行、真正一局會多的三件事、開始六人局／再跑一次。
- **動畫**：ComfyUI 的 MiniMax H3 turbo（`dual_clock_4step_api` 節點鏈，T2VA＋native 音畫同生）產生三段 15 秒、一段 10 秒開場；用 ffmpeg 壓成 640 寬 CRF30、音訊 64k 單聲道，三段共約 900 KB，放在 `public/video/`。播放點：按「記住了，開車」後播上車（單人是播完才開局；包廂是先送出就座再播，免得別人等）；一局結束依輸贏播下車或癱坐。全螢幕、點畫面或右下角「跳過」可跳過，設定頁有開關，也尊重系統的「減少動態」。
- 過程中修的兩件事：事件卡的 class `ev` 和行車紀錄列撞名造成版面塌掉（改名 `stopcard`）；全螢幕層放在表格視圖裡會被 `.scr.table > *` 壓回文件流（移到視圖外）。
- 驗證：瀏覽器把導覽對局從頭跑到尾（六張嚮導、攤牌成功、結算頁、贏的動畫自動播出），中英文鍵值一致，部署後 8 個檔案逐位元比對一致。提交 207b9c5，版本 bbadf85c。

## 動畫交接、結算節奏與 AI 快慢（2026-09-16 上線）

- **一段影片怎麼交接**：影片播完 → 漸黑 1 秒 → 遊戲（或月台）在黑幕後面換好 → 漸亮 1 秒 → 再等 1 秒才有人動。時間都在 `FADE = { black, lift, settle }`。提交 c6fa808、3af10be。
- **單人局結束回月台**：結算動畫播完後直接回首頁（`leaveRoom` + `go("")`）。包廂維持停在結算頁（別人還在，那張誰是哪幫、誰有什麼的表要看得到），教學局維持停在結語頁。
- **結算要先按「知道了」才播結束動畫**（車主要求）：一局結束後結算頁停著，下面一顆「知道了」；按下去黑幕蓋上 1 秒 → 影片從黑裡浮上來 → 播完照上面的交接回月台。動畫關掉時，這一按就是單人局回月台的那一按（先前會自己跳走，結算表一閃而過）。提交 3c8965c。
- **直向手機的影片改成帶狀**（車主要求）：影片是 640×370，直立手機填滿高度會吃掉約四分之三的寬度。`@media (orientation: portrait) and (max-width: 700px)` 下影片高度改 60%：375×812 上是 375×487、上下黑條各約 4.3 公分、看得到的畫面寬度從 27% 變 44%。橫向手機、平板直向與桌機維持滿版。用比例而非固定公分，不同尺寸的手機裁切比例才一致。提交 f3f3312。
- **人物介紹加幫會信物**：點乘客的說明卡，幫會欄位改成信物圖（懷錶／印信，`art/item_watch|item_seal.jpg`）加幫會名，尺寸與行李物品一樣（56px 方框），外框用幫會顏色。沒看過的人照舊是問號。
- **AI 節奏放慢**（車主：太快）：`public/app.js` 的 `DELAY` 與 `src/room.js` 的 `BOT_MS` 各加一個 `PACE = 1.5`，每一步的停頓都乘 1.5（單人與包廂一致）。提交 4e2d4bf。
...
## 行車紀錄 · 車長筆記（2026-09-20 上線）

- 設計在畫布第十九頁，三案：A 站牌（摘要＋衝突計分卡）、B 車長筆記（夾私人筆記＋篩選）、C 行李流向（誰給誰＋到站小結）。車主選 **B**。
- **分站收合**：每站一組，站名就是把手。列車所在那站預設展開，過去的站收成「6 手 · 2 則筆記」，點一下打開；最新的站在最上面，站內照順序、每一手有編號。站名旁的標籤是那站的事件（沒開站站有事就沒有標籤）。
- **更詳細 = 只有你知道的事**：引擎的 `knowledge` 本來就帶回合編號（`at`），直接夾在那一手下面的虛線銅框裡：看過的行李、幫會牌、行當、對方給你／你給對方／被拿走／遞過來被你拒絕的東西、算命師看到的牌堆。全車都看到的（暗號、餐車）已在公開紀錄，不重複。旁觀者沒有筆記。被接受的交換只記「給你」，不再重複記「遞過來」。
- **篩選**：全部／衝突／交換／我的筆記／關於某人（點開一排頭像選人，再點一次取消）。篩選時有結果的站自動展開、沒結果的站不顯示。
- 每次車廂有動作清單都會重畫，現在會記住捲動位置。新一局（單人、教學、包廂換局）會重設收合與篩選。
- 沒動引擎與規則；`public/app.js` 的 `renderJourney`、`notesByTurn`、`entryTags`，樣式 `.jstop/.jhead/.jturn/.jnote/.jchip`，`i18n` 新增 `journey` 區塊（中英鍵值 472 對 472）。舊的 `.ev`／`.stopline` 樣式移除。
- 驗證：瀏覽器跑自動對局，五種篩選都量過列數與筆記數、中英文都看過；39 個測試全過；部署後四個檔案逐位元一致。提交 b955f24，版本 c49ea667。

## 頭像閒話氣泡、AI 節奏、誰看過誰（2026-09-25 上線）

設計在畫布第二十頁。車主選：1 用 A＋B、2 照提議、3 用 A。提交 5813da7，版本 f4b85012。

- **閒話冒到頭像上**：有人說話（機器人台詞、你的快捷句、包廂聊天）就在說話者身上顯示 4 秒。**六人以下**把那句話放在氣泡裡，氣泡往圓桌外側偏、不擋桌心「誰的回合」，同時最多兩個。**七人以上**只冒一個「…」記號，點記號才打開那句話。完整的話照樣進車廂閒話。實測六人 46 個氣泡：沒有超出畫面、沒有擋到桌心、沒有蓋到任何頭像。
- **AI 節奏**：等待改成看「上一步剛顯示了什麼」決定，規則放在 `public/shared/pace.js`，單人與包廂共用。畫面沒變 0.3 秒、表態 1.2、開始一件事 1.5、過／無事 1.5、亮牌 1.8、一般紀錄 2.5、大事 4 秒；看完結果卡按「知道了」後停 1 秒。模擬：一局裡約一半的 AI 步驟畫面上什麼都沒變。AI 等待時間 4 人 7.4→9.5 分、6 人 8.8→10.4 分、8 人 11.6→12.8 分。舊的 `DELAY`／`PACE`／`BOT_MS`／`flashUntil` 移除。
- **誰看過誰**：行車紀錄多一個分頁，可切「被誰看過／看過誰」。只根據全車看得到的事推算：衝突贏家選「看」、換出單片鏡、密碼本互換、行當公開；你自己看到的寫出內容，別人只顯示「看過」。看過之後對方又換了行當的會劃掉。推算放在 `public/shared/seen.js`。
- 測試新增 `tests/pace.test.js`、`tests/seen.test.js`。seen 測試拿引擎自己記錄的每席知識對照，兩個方向都驗（不多報、不漏報），含擴充包對局；拿掉單片鏡規則時測試確實會失敗。共 44 個測試全過，部署後 6 個檔案逐位元一致。

## 最後一件行李規則（2026-09-25 上線，照原版修正）

- 原版規則書：衝突贏家拿走的若是輸家**最後一件**行李，贏家要從自己的行李挑一件還給輸家（„War dies die letzte Handkarte des Unterlegenen, muss er ihm einen beliebigen Gegenstand aus seinem eigenen Besitz zurückgeben."）。當初整理規則摘要漏了這句，遊戲一直讓贏家直接拿走。
- 修正前的機器人對局（各 200 局）：約四分之一的「拿走」拿的是最後一件；4／6 人局 72%、8 人局 83% 的局裡有人曾經空手，空手就不能交換、不能攤牌。
- 現在：拿走、搬運工自己挑出、交出的金條，只要讓輸家空手，衝突就多一步 `giveBack`，贏家挑一件還他，哪一件都行，連剛拿到的也可以。公開紀錄只說「還了一件」，不說是哪件；輸家的結果卡會寫還回來的是什麼。手上只有一件時自動還。機器人還最不值錢的那件。
- 測試 `tests/lastbag.test.js`：新步驟本身，以及 3～10 人、含擴充與事件，每一回合開始時人人至少一件行李；把規則關掉時測試立刻失敗。另外修了一個小問題：用金條打發的贏家，結果卡不再把舊的偷看結果當成新的顯示。
- 規則頁中英、規則摘要（rulebook digest）都已補上。驗證：51 個測試全過、覆蓋測試（擴充＋事件）160 局 0 引擎錯誤、瀏覽器實際操作「還他一件」畫面；部署後 6 個檔案逐位元一致。提交 c62e1b7，版本 6a0df0c4。

## 版本檢查、賽後匯出，語音定案（2026-09-25）

- **語音聊天**：設計在畫布第二十一頁。車長決定預設 **2A 按住說話**。尚未實作；待查轉發服務（Cloudflare Realtime）費用、iPhone Safari 背景時停麥克風。
- **版本檢查**（已上線，提交 e55c32c）：`tools/stamp.mjs` 把所有原始碼雜湊成 `public/shared/version.js`（產生檔、不進 git），每次 `wrangler deploy`／`wrangler dev`（`build.command`）和 `npm test` 前自動產生。包廂在 `joined` 帶上版本；頁面版本不同就跳一張卡「車廂換新了」，重新整理後回到同一個座位（token 在分頁的 session 裡）。實測：開包廂、在底下改一個伺服器檔，重連後幾秒內跳卡，重新整理回到原座位。單人局不需要。這次部署之前就開著的頁面聽不懂新欄位，從下一次部署起才有效。
- **賽後匯出這局**（已上線，提交 0d1b382，版本 5a2cb981）：設計在畫布第二十二頁，照建議做。結算頁「匯出這局」→ 格式（行車紀錄 .md／重播檔 .json）× 內容（完整攤開／只有我知道的）→ 複製、分享、下載。
  - `.json` 完整版＝種子＋每一步＋公開紀錄＋終局攤牌，`public/shared/record.js` 的 `replay()` 能原樣重跑；自己視角版沒有種子與步驟，不能重播。
  - 單人預設完整；包廂伺服器也記下種子與每一步，但只有車長在大廳勾「賽後公開完整紀錄」才交出去，否則只能匯出自己的視角。部署前就開始的局沒有完整紀錄，會明講。
  - 測試：`tests/record.test.js` 24 局（3～10 人、擴充、事件、走私）經 JSON 重播結果完全相同；`tests/room.test.js` 走真的包廂程式打完一局，未開放時被拒、開放後交出的紀錄可重播。瀏覽器實測中英文、兩種格式與內容，匯出的 JSON 在瀏覽器裡重播相同。包廂這一側的「匯出」畫面沒有實際開一局包廂玩到底測過（伺服器協定有測）。
- 測試共 51 個全過。
