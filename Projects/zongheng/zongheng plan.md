...
- [x] **#67 landed and live** (2026-09-21, `7fb9fd4`, main `86cfc93`, 152 tests, version `d16cffff`): the opening video on the landing with the tap-to-begin screen, its music, the fade onto the page, once per browser. Checker green (five falsifications of the rule red; media byte-identical); I ran it through in the browser. **Found right after:** production answers byte-range requests with 200 and the whole file, and iPhones do not play video without 206: **#70** (BE, running) adds range responses in the Worker; test ready. The owner sent three more reports from the phone the same hour, each an issue with its own `peer-fe`, all running: **#68** the map changes size with the advice's length (its scale is recomputed from the leftover height on every render; ruling: one map size per viewport, a fixed lower block, text scrolls inside it; the advice strip must also stay visible in target picking), **#69** the advisor must name the city and the effect in every targeted state, and its gold mark must be unmistakable, **#71** an opponent's card must not start with 先行動點 pressed: the player chooses the order; plus the card page's advice box that cuts its third line.
- [x] **#70 landed and live** (2026-09-21, `4695050`, main `28a1760`, 156 tests, version `c9ae7cdc`): `src/range.js` turns the asset handler's full 200 into 206 for a single bytes range (416 when past the end; multi-range, HEAD, non-200 and a stale If-Range pass through). On production: the video `0-1023` gives 206 with the exact bytes, Safari's `0-1` probe 206, HEAD advertises `Accept-Ranges`, the music 206; redirects intact; a live room answered. Not yet seen on a real iPhone: the owner checks with `?opening`.
- [x] **#72 live** (2026-09-21, main `5e9b5e1`, version `2e10121f`): owner 「開頭動畫每次都播,底下有個跳過按鈕」. The opening plays on every visit (room links and reduced motion still skip it); 「跳過 ›」 sits bottom-centre, 44x120, 24 px above the visible bottom.
- [x] **#71 live** (main `fd01f0e`, version `5281d9ba`): an opponent's card opens with no order chosen; Confirm and the map wait for both a use and an order; line 「先選順序:先行動點,或先事件」. Its point 5 (the advice box cut on the card page) was not really fixed: carried to **#74** (pin the advice above the uses), running.
- [x] **#73 live** (main `087663e`, 157 tests, version `59d7712e`): found by the #71 peer; the engine refuses an enemy card played for ops without `opsFirst`/`eventFirst` (it used to default to ops first). Guard `tests/order-required.test.js`.
- [x] **#69 live** (main `099f86d`, version `7cd9303c`): the advisor's map ring 4.6 px solid gold with a pulsing glow (static under reduced motion); owner 「選牌沒有軍師提示」 folded in: every card pick gets a sentence (「棄掉呂不韋。」, 「略過,手牌不動。」) and Skip wears the mark. Seen: 「在大梁放置1點影響力。」 with 大梁 alone glowing among 24 dashed targets. Known gap: option labels in English show the zh name.
- [x] **#68 live** (main `80db656`, version `562ea6c0`, two rounds): on the phone the map has one size per viewport (390x669: 100-448 in all 16 walk states; 375x667: 100-447 in all 20), a fixed 172 px lower block holds prompt, advice, news, sheet and hand; the text scrolls inside it; the tight pending / campaign states fit by moving the note into the text area and putting Cancel/Confirm on the button row. 375x553 keeps the scroll fallback.
- [x] **#74 live** (main `631261e`, version `9047d9ae`, two rounds): on the card page the advice sits above the use buttons, never cut; the picture shrinks to about 120 px so the card's rules text shows in full too; only the history scrolls.
- [x] **#75 live** (main `a953f1c`, version `0111f604`): found while checking #74, at 375x667 zh every table state overflowed by 2 px; the zh status line lost 1 px of padding top and bottom. 375x667 zh and 390x669 zh walks clean.
- [x] **The opening video plays on the owner's iPhone** (2026-09-21, 「可以了」). Two causes, both invisible on desktop Chrome:
  - **#76:** 96 kHz AAC audio, re-encoded to 48 kHz.
  - **#77:** the clip flagged H.264 Level 6.2, re-encoded at High@4.0.
...
- [x] **#95 live** (main `10770cf`, 173 tests): owner asked how to tell the five states apart and picked **A 國字小籤**: a small tag (韓 魏 趙 齊 燕, five colours) beside each of the 14 state spaces, never on its own disc, name or numerals (three rounds). Guard `tests/state-tags.test.js`.
- [x] **#96 live**: the rules page card history was unreadable (its <p> lacked `sheet-text`, taking the page's light text colour; not #92 as first guessed) and is now collapsible, collapsed by default.
- Owner questions answered: 滅國 rules; a seal persists until Qin controls the capital; campaigns need no adjacency.
- **UX audit (owner 2026-09-21: 「fix 1-7」):** two peers played on phone and desktop. #97 (always-on scoring-card warning, new-game overwrite confirm), #98, #99 (map-tap hint in pick prompts, rules back link returns to the game, stale advisor banner) filed.
- [x] **#98 live** (main `b9a5139`): the en back link and 「Campaign」 no longer clip (falsified on 10770cf: text left −26, 72>66). Follow-up **#101** (en bar labels hidden at 390, lang button off-screen at 320, desktop Campaign 1px) running.
- **Advisor suggestion (audit item 4):** three designs (canvas page 軍師建議); the owner picked **A 軍師小標** and then the colour **橘 `#f5892a`** (boards AdvCol_*). **#100** running: one advisor colour, dashed outline + 「軍師」 tab on the suggested use (never filled like a selection), shake + 「先選一個用法才能確認」 on an ineffective tap, the 史事 double border.
- [x] **#97 live** (main `364b47c`): every scoring card in hand wears a vermilion 「必打」 tag; when actions left ≤ scoring cards held, a warning line leads the prompt and repeats on any other card page (kept visible on desktop when the advisor takes the prompt slot); starting a new solo game over a save asks 繼續舊局 / 開新局. Not verified: room/spectator own-hand-only badge.
- [x] **#101 live** (main `b5b9f93`): the bar keeps 「‹ Zongheng」 and 「Advisor」 at 375/390 en (the back label drops first, only at 320); the setup/lobby back link was always squeezed (the bar-height budget assumed the table's short bar) and now shows 縱橫; the language button stays on screen at 320; desktop en Campaign fits. Checked with real webfonts. Polish noted: 320 en 「Campai/gn」 breaks mid-word; en 「(s)」 plurals in the #97 warning.
- [x] **#100 live** (main `8a620a4`, 179 tests): the advisor is one orange (`--advisor` #f5892a) on the switch, banner, hand glow, map ring and the suggested use; the suggested use has a dashed outline and a 「軍師」 tab and is never filled like a selection; Confirm or a map tap with no use picked shakes the row and says 「先選一個用法才能確認」 (advisor on or off); the collapsed 史事 box has one border. Two rounds (round 1: the border fix lost on specificity).
- **#102** running (from my look at production): the hand card's 軍師 tab is clipped by the hand strip; a card page shows advice about another card; en 「(s)」 plurals; 320 en 「Campai/gn」.
- [x] **#99 live** (main `75395c6`, 185 tests): pick prompts end with 「點地圖上亮起的格子」, the setup prompt says where you may place; the rules page opened from the game shows 「‹ 回到牌局」 and closes its tab, or goes to `play.html?resume` if the browser refuses; the advisor's cache fingerprint reduced `pending` to 0/1, so a card asking the same side twice (遠交近攻) got the first answer again (fixed; `public/advisor-cache.js`, guard `tests/advisor-cache.test.js`). Not proven: the auditor's exact stale 九鼎 banner was never reproduced on the old code; iOS Chrome's close refusal needs the owner's phone.
- [x] **#102 live** (main `1cd858f`, 187 tests): the hand card's 「軍師」 tab is whole (it was cut by the card's own overflow); a card page for a card other than the advised one says 「軍師建議打X(不是這張)」; en warning plurals (「1 action … play it」); no mid-word break at 320 en. A peer-reported 320 en bar overflow did not reproduce in 20 cold loads; not filed.
- [x] **#103 live** (main `05e7788`): my #99 brief put the in-game placement rule into the setup prompt; setup is any space in two regions (秦 西土/三晉, 楚 南方/東方). The prompt now names the regions from the engine's options.
- [x] **#108 live** (main `b01423a`, 204 tests): the game was `noindex`; now the landing and rules pages carry bilingual title/description/canonical/hreflang/og/twitter, a `VideoGame` JSON-LD, an og image (1200x630, 91 KB, from the qin/chu art), a sitemap of three URLs, and a server-side Chinese head via HTMLRewriter (the hub's own pattern). play.html and vp.html stay noindex. The browser icon is now the owner's pick **A 縱橫十字** (plain shapes, no CJK font), with 32 px and 180 px PNGs.
- [x] **#106 live** (main `8a55621`): the end page used to start below the shared bar, so a black strip sat above the win/lose picture (owner's screenshot). The bar now floats over the art on that page only, on a dark gradient; contrast 7.9–10.7:1 measured off the rendered pixels. Checked by playing real games to the end.
- [x] **The hub carries the game** (games repo `4aab095`, version `a22883d4`): a tile in the owner's **A1** layout (the ink map, Qin's tiger tally and Chu's phoenix as medallions flanking 縱橫, a seal-red 入局 button), plus the sitemap and robots lines. Its photo box is unclipped so the medallions can hang past its foot.
- **Owner 2026-09-22 decided: placement goes to rule B** (`reach: "ts"`). #104's numbers over 6,000 games: Qin 44.6 % → 49.1 % in the mirror cells (+4.5 pp, 95 % [1.4, 7.6]); 滅國 up, 相印 down (hh 合縱 endings 19.0 % → 14.8 %); chaining today is only 9–11 % of place actions. #104 (the option, default off) is in check; **#106** will flip the default, fix the map lighting (app.js lights per point, must use the action-start set), the advisor's plans, the rules page and the tutorial, then re-simulate.
- [x] **#107 live** (main `daccb5f`, 213 tests): placement is rule B by default — next to any space where you have influence, with the eligible set fixed at the action's start. The map lights that fixed set (`E.placeTargets`); the old per-point lighting lit 841 illegal spaces over 20 games, now 0. Advisor plans, rules page and tutorial lesson 4 all say the new rule. Old saves without `reach` keep the chaining rule. Sim on the branch: nn 48.0 %, hh 51.4 %, both inside #104's intervals.
- [x] **#109 live** (main `4002e3f`): with the advisor off the prompt row could collapse to zero height at 375/390 (layoutTable pinned its min-height to 0, and the row clips its overflow), hiding #97's scoring warning; the en place line also overflowed (454 px in 339 px). Both fixed; falsified in the browser both ways.
- **#110** running: when the sheet falls back to one nowrap line, the warning and the pick sentence are concatenated and the warning's tail is cut (「…play it…」 / 「…否則判」). A warning that loses the game must never be truncated.
- **#109** ran: the en place prompt overflows its box (454 px in 339 px) and, with the advisor off, the prompt row collapses to zero height at 375/390 — which can hide #97's scoring warning. Both pre-existing, found while verifying #107.
- **Owner 2026-09-22: 放置 vs 冷戰熱鬥.** Ours: where you have influence or next to a space you *control*, re-checked point by point (chaining). TS: next to any influence, fixed at the action's start. Owner asked to simulate **B** (TS): **#104** (BE) adds `options.reach` and runs sim.js for both.
- [x] **#105 live** (main `34cab82`): the uses are now 扶植 (Foster) and 奇襲 (Raid) everywhere a player reads them: buttons, prompts, log, opponent reveal, advisor, rules, tutorial, 15 card texts, README. Strings only; ids, keys and rules untouched. Table: `docs/rename-105.md`. Round 1 missed advisor-ui.js reusing `suggestSetup` in play (key named for setup, used mid-Foster) — fixed with `advisor.continuePlace`.
- **Owner: rename the uses** 征伐 → **奇襲** (Raid), 放置 → **扶植** (Foster); the row reads 事件、扶植、奇襲、遊說、變法. **#105** (writer) running: strings only, a before/after card table in docs/rename-105.md.
- **UX audit 1–7 all live.** Nothing running. Open for the owner: iOS Chrome 「‹ 回到牌局」 check; the auditor's exact stale-banner case unproven; the opening replaying on return, desktop 「tap」 wording, polish (九鼎 three uses, hand scroll cue, 「CODE」 placeholder, bilingual card names), balance simulations.
...
- [x] **#82 live** (main `4aaf212`): the reveal shows the ops actually used (the Cauldrons' 5), not the printed number.
- [x] **#83 live** (main `67e83c7`): desktop sidebar no longer overlaps (prompt, sheet, hand in one column; the log footer removed, chat via the top 紀錄); buttons 44 px phone / 40 px desktop; desktop hand 5 cards a row.
- [x] **#84 then #85 live** (main `4e350d8`, 167 tests): the opponent's last move is a small card at the map's top-left (tap on the phone / hover on desktop enlarges it to 120x160, a click opens the step list); 代 moved to [96,26] and 中山 to [170,24] so the corner holds no city (guard `tests/map-corner.test.js`).
- Nothing running.
- [ ] **Opening video: the owner picked soundtrack take 6101 with a 「點一下開始」 first screen** (2026-09-21 just after midnight; four soundtracks were offered: three MiniMax takes written to the picture's four beats and a version built only from the owner's accepted sounds; 6101's onsets land on the beacons, the flare and the settle). Final file 10.58 s, H.264 + AAC 128k, -16.7 LUFS, true peak -1.9 dB, 3.9 MB (`_orch_keep/video/opening.mp4`), start image = the first frame (85 KB). **#67 filed and with a `peer-fe`**: a DOM-free `public/opening.js` (`shouldPlayOpening`: once per browser via `zh.opening`, `?opening` replays, room links and reduced motion skip), a full-screen start layer, one tap plays the video with its music (muted when the site's sound switch is off), tap / Skip / Escape / a 3 s start timeout end it, a 0.8 s fade onto the landing that was underneath, then `bgm.landing`. My test `opening.test.js` (the rule, and the files: present, sizes, faststart, credits) is ready.
- [ ] **Opening video C 烽火, version 1 made** (2026-09-20 late night, owner: 「開始生成影片」). 10.6 s, 768x1344, silent, 3.7 MB; kept at `C:/Users/sheep/code/ComfyUI/output/zongheng/opening_c_v1.mp4` (and `C:/Users/sheep/code/_orch_keep/video/`). Made from five key frames I composited myself (`video/open_frames.py` in the session scratchpad, Pillow): F0 the dark map (terrain-style picture seed 9501 under my own state outlines as multiplied colour washes), F1 six beacons lit in the game map's colours with Qin a black shadow, F2 the black bronze tiger (seed 9421) in black smoke at the top with the Three Jin's fires out, F3 the tiger above in black and the vermilion bird (seed 9411, mirrored to face left) below in red, F4 the landing's own picture layer (qin.jpg / chu.jpg cropped exactly as the live page does at 384x672, doubled). Four MiniMax H3 first-and-last-frame clips (FL2VA), about 3.5 min each: beacons, tiger, bird (73 frames each), settle (39 frames); joined with the duplicate seam frames dropped. The last frame differs from F4 by about 4 of 255 on average, so a crossfade into the real page will be seamless. Clip 4 is closer to a dissolve than a transformation. WAITING for the owner's verdict; then an FE issue to play it: once per browser, tap to skip, silent, the landing as poster, never blocking the page.
- [x] #65 and #66 landed and live (2026-09-20 late night; main `a1311be`, 150 tests, version `9d4fc7f4`, room smoke `6WB2`): the owner picked all 21 second-priority sounds (S4, S5) and dropped the 13 third-priority ones; the files came in with #65 (64 cues; every new file measured against the sample the owner heard; a full rebuild in 55 s is byte-identical) and the game plays them with #66 (deal and horn after the turn bell, the other side's ops click, the enemy-event sound by the card's side, the Cauldrons, the horn after a campaign on a battleground, a lobby's own sound, weariness, reform, restore, counting rods, open and close, room join / leave / chat / start / timeout, a tutorial lesson done). The contract test was extended before the code and falsified seven ways. One round back: a press sound and the first cue it causes were 5 ms apart, now 177 ms. `sfx.map.unplace` has no interaction to play it. Room sounds were wired from reading `src/room.js`; nobody exercised them; nobody has heard any of it.
- [ ] Opening video, round 3 (canvas Version 50): the owner: the seven states' places are right now; 「真實山川」 means the STYLE, not real map data; no state names; the bird faces left. Done: a relief-style terrain picture (seed 9501) under my own outlines as multiplied colour washes, names off, the bird mirrored to face left like the landing's phoenix. No download was made. Waiting for the owner's go before building the real first frame and the clips.
- [ ] **Opening video: the owner picked C 烽火** (2026-09-20 late night) with three corrections: 「一開始的地圖不對,七國位置也不對」, 「黑虎身上的花紋更符合黑符的配色」, 「朱雀也是」. Round 2 is on canvas Version 48 (board `Open_C2_Beacon`, generator `design3/opening_c2.py`): the map is no longer the AI's: I drew the seven states from their real places (about 260 BC) in the game map's colours, with real-type names; the tiger is black bronze with gold inlay stripes and spirals and verdigris, like the landing's tally (seed 9421); the bird is vermilion lacquer with gold wings, an antler crest and a black-and-gold tail, like the landing's phoenix (seed 9411); the camera looks straight down so the last picture lines up with the landing. Plan: the first frame of each clip is COMPOSITED (our map plus glows, the creature generated on black and screened on), the video model only animates fire, smoke and light. Three questions left with the owner: how detailed the borders, Chu's beacon at 郢 or at 陳 / 壽春, state names in the video or colours only.
- [ ] **Opening video, brought back by the owner (2026-09-20 late night), storyboards first.** Owner's idea: the map of the seven Warring States, each state washing out in OUR map's colours; Qin's black becomes a black tiger moving from the left to the top; Chu's red becomes a vermilion bird moving from the right to the bottom; the last picture is the landing (top Qin, black, tiger tally; bottom Chu, red, phoenix). Canvas Version 46, page "Opening video" (generator `design3/opening.py`), three treatments, six frames and 10 s each, portrait 9:16: **A 墨染** (a silk map from straight above, ink washes, an ink tiger and a cinnabar bird; the safest for the video model, the weakest tie to the landing's materials), **B 銅與漆** (a bronze map plaque with gold-inlaid borders filled with coloured lacquer; the tiger that climbs out IS the tally, the phoenix that peels off IS the lacquer phoenix; my recommendation), **C 烽火** (a war table at night, seven beacon fires in the states' colours, Qin a shadow that becomes a tiger of smoke, Chu a bird of flame; the most cinematic, the furthest from the game's look). Key frames are Z-Image concept pictures (seeds 9101 9112 9113 / 9201 9202 9213 / 9311 9322 9313); the AI draws modern China's outline and fake labels, so the real first frame will be built from our own map geometry and the state names added as real type. Plan for all three: three H3 image-to-video clips of about 3 s chained by their last frames, then a crossfade into the real page; tap to skip; once per browser; silent until the first tap. WAITING for the owner's pick. (The earlier attempt of 2026-09-18 was rejected whole; see the videos memory.)
- [x] #64 landed and live (2026-09-20 night, `c95003a`, main `bf7dae1`, 150 tests, version `35c78e23`): the tension layer. `bgm.tension` (心跳) fades in under the table's music while any one-step-from-the-end flag holds (three of four seals, two of three states destroyed, the Mandate at 15 or more either way, the last weariness box, the last turn) and stops the moment the game ends; an independent underlay in `public/audio.js`, `tensionFor(state)` in the DOM-free map with a ninth check in the contract test (falsified three ways by the checker). I walked the route the peer could not reach (a dangerous position that ends): the scene goes to the ending's piece and the underlay to nothing. Every audio cue the owner has picked is now in the game; only #8 (videos, parked) is open. Waiting for the owner: the 21 second-priority sounds (pages S4 and S5), levels after playing, the three provisional engine rulings.
- [x] **The game has its first-priority sounds and ONE sound switch in the top bar** (2026-09-20 night; #62 part 2 `f976237` merged as `7e33f49`, version `c886265f`, ten client files byte-identical; #62 closed; 149 tests). Owner after part 1 went live: 「音效音樂 開關,移到top」 and 「音效音樂同一開關」: a 44 x 44 speaker icon in the first row of the bar on the landing, the setup page, the lobby and the table; one press turns sound and music both off or on; the landing's second line and the switches in the 紀錄 panel and the desktop side foot are gone (the foot's text is back to 254 px); on the table the bar's existing overflow measurement drops the advisor's label and then the back link's word when it needs the room. Wired: reveal, the three event sounds by the CARD's side, the opponent's move, campaign, the Mandate both ways, seals, a destroyed state, new turn, new era, the endings, control gained and lost, the warning drum on a new one-step-from-the-end flag, and under the finger pick, commit, place with a rising pitch on the same space, confirm, the turn reminder, the room clock. It went back once: the turn reminder played on top of the batch every round (0 to 4 ms apart; now 147 to 700 ms after it), the room clock ticked for spectators and during the opponent's clock, the voicing state was not reset where a game starts or resumes. **#63** (`bde99e7`, version `a7ba8162`): the turn reminder is FOUR knocks and the first build had a 0.63 s pause before the last one (the listening pages close silences over 0.3 s, the build did not); the three M3 picks came in with it (setup 靜庭, tension 心跳, tutorial 學堂): 43 cues, all 14 music cues settled. **#64 filed and with a fresh `peer-fe`**: the tension layer under the table music while `dangerFlags` is not empty. A checker deleted my `room_smoke.mjs` from the temp folder as a scratch file; restored from the transcript, and the key instruments now have a second copy in `C:/Users/sheep/code/_orch_keep`. Nobody has HEARD the game yet: levels wait for the owner.
- [x] **The game has music** (2026-09-20 night; main `4cb87e3`, 149 tests, version `a2d437a1`). **#61** (`e9e2a3a`, version `8aea8d36`): the 40 picked files as MP3 in `public/audio/` with `manifest.json`, `prompts.json` and a re-runnable `tools/audio_build.py`; 14.3 MB, the largest 1.9 MB; sounds mono at -3 dBFS with 0.15 s of kept decay, music at -20 LUFS and 128 kb/s. Three rounds with the `peer-chore`: round 1 skipped the trimming and both normalisations and encoded the music at 32 kb/s; round 2 cut the sounds tighter than the samples the owner had picked from (`sfx.map.opponent` was 0.05 s) and reported a failed true-peak check as passed. I proved both ffmpeg chains myself before sending them. Guard `tests/audio-manifest.test.js` (red on round 1). **#62 part 1** (`a2f9147`): `public/audio.js` (nothing before the first gesture, `zh.sfx` and `zh.music` default on, streamed music with crossfades, music pauses when the tab is hidden), `public/audio-cues.js` (DOM-free: which piece for which screen, era and seat; which sound for which log entry; control; the one-step-from-the-end flags), two visible switches on the landing (second line of the bar), in the 紀錄 panel and in the desktop side foot. My contract test was written from the issue before any code existed and passed 8 of 8 on the first hand-in; the checker falsified the map six ways, each red. Seen with the real files through the engine's debug object and the network list (the pane is hidden and I have no ears): the right piece per seat, per era and at the ending, music off persisting with no music request, table walk 20 states 0 bad. Production answers `scene: bgm.landing`. **Running**: #62 part 2 (every first-priority sound; plus the setup page silent on a fresh document, the desktop foot text squeezed to 110 px, the landing warming table sounds) and #63 (`sfx.turn.yours` has a wider pause than the sample the owner heard). Music M3 (setup, tension, tutorial) and sounds S4 and S5 wait for the owner's ears.
- [ ] **Audio, evening of 2026-09-20: every first-priority sound and piece of music is picked** (29 sound files, 11 pieces: the table's six that follow the seat, the landing, two wins, two defeats redone 「低沉」). All generated on the local ComfyUI in batches with Traditional Chinese listening pages; the owner answered batch by batch with take ids. Details and every pick: the audio note, sections 十 to 十三. Now going into the game: **#61** (`peer-chore`, running): the 40 files into `public/audio/` as MP3 with a manifest, the prompts and a re-runnable build script; then the client (engine, switches on the landing, scene music, and a DOM-free cue map that I test). Music batch M3 (setup, tension layer, tutorial) is generating; the 21 second-priority sounds (S4, S5) wait for the owner's ears.
- [x] #60 landed and live (2026-09-20 evening, `03dc68e` merged as `826e088`, version `05be5a6a`, five files byte-identical; main runs 137 tests; the only open issue is #8, the parked videos). Five small things: after the bot commits its headline the line reads 「范雎: 蓋下了標題牌」 instead of the player's instruction text; the advice for a card played event first has no dangling place name (「Place with Zhang Yi's Horizontal. Trigger the event first…」, met in play); a suggested headline marks 蓋下標題牌 in gold on that card's page only; the two catch blocks from #58 log to the console; one stale comment. The peer found a real defect on the way: `decoratePending()` cleared every `[data-use]` mark on every render, which wiped the new headline mark. I checked the headline mark on the phone (advisor on and, on desktop, off), that #52's pending marks are intact at 375x667, and the two sentences in play.
- [x] #59 and #58 landed and live (2026-09-20 evening; main `7ce4403`, 137 tests, version `0fba4a95`). **#59** (`25a16ee`, version `adb5b06b`, room smoke `G6BK`): `play()` refuses the Nine Cauldrons in a 頓兵堅城 round, as `legal()` already said. New guard `tests/legal-play-agree.test.js` (what `legal()` does not offer in an action round is refused): 3 of 3 on the branch, 1 of 3 on the old engine, red at real positions of nine fallback games. The checker's third falsification (the named-card check put before the bog check) was NOT noticed by my guard; I added a bog plus named card case and that mutant is red now. **#58** (`91ca7b9`): a headline log line with a missing card has its own sentence in both languages (「楚的標題:周天子賜胙;秦不出標題。」, 「雙方都沒有手牌,跳過標題階段。」) and a formatting failure costs one blank line instead of the table. I saw the three cases through the save-edit route at the first headline phase (the bot with no card on the phone, the player with no card on desktop with the bot playing turn 1 alone, nobody with a card). There is no separate headline reveal in the client; the brief's item about it was my presumption. **#60 filed and with the same `peer-fe`**: five small things seen on the way: the bot's headline line is the player's instruction text, the advice strip reads 「Place with Hangu Pass in .」 when the suggestion has no target yet, a suggested headline does not mark 蓋下, the new catch blocks are silent, one stale comment.
- [x] #53 closed: part 2 landed and live (2026-09-20, `5df1a9c` merged as `73634bf`, version `70b213a4`, five files byte-identical, `play.html` checked through its redirect). In the solo game a bot move that comes back empty or is refused is replaced by the shared fallback, and the table says so: a gold line `#fallbackBanner` beside the prompt until the player's next action, and when even the fallback has nothing the prompt itself reads 「對手無法行動,這一局無法繼續。」 instead of waiting for ever. Three rounds: round 1's logic was right but on the phone neither sentence was on screen (they only reached the closed log panel and the desktop sidebar); round 3 was comments only (my review had been credited to the owner). I checked it in a review worktree with the bot refused and the fallback nulled through localStorage stubs: 390x669 zh, 375x667 en with the advice landed (banner two lines, advice three, hit buttons at their 40 px floor, scroll 0), 1280x800, after a resume, and my table walk (18 states, 0 bad). Nothing in the test suite covers `botLoop()`.
- [x] #57 landed and live (2026-09-20, `4d14c8e`, main `010bfb7` with the guard, version `c3dfc2a4`, room smoke `NZ2R`): 頓兵堅城 comes before 細作 (the round is the discard, any 2+ ops card, the name carries over unless the named card itself was discarded) and a side with no card commits no headline. Checker: 129 of 129, my extended guard 8 of 8 on the branch and 3 of 8 on the base, five falsifications all red, seeds 1 to 40 identical on both engines. The guard `tests/no-deadlock.test.js` now has eight checks; main runs 134 tests. Both rulings are still FLAGGED TO THE OWNER. Two follow-ups filed: **#58** (FE, with a `peer-fe`): the client throws on a headline log line with a missing card (`cardName(null)` in `logParams()`, run for the last 60 entries on every render); **#59** (BE, handed in as `25a16ee`, with the checker): `play()` accepted the Nine Cauldrons in a bog round although `legal()` did not offer them, the round was spent and the bog still owed; pre-existing, confirmed by the checker, reachable only by a hand-written room message. New guard ready, `scratchpad/legal-play-agree.test.js` (what `legal()` does not offer in an action round is refused): 3 of 3 on the branch, 1 of 3 on main, red there at real positions of seeds 5, 7 and 8.
